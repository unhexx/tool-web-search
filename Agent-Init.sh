...
# Источник: agentic_loop_template/examples/consumer-starter/Agent-Init.consumer.sh
set -euo pipefail
...
if [[ -n "${AGENTIX_TEMPLATE:-}" && -d "${AGENTIX_TEMPLATE}/memory" ]]; then
  TEMPLATE="$(cd "$AGENTIX_TEMPLATE" && pwd)"
elif [[ -d "$ROOT/../agentic_loop_template/memory" ]]; then
  TEMPLATE="$(cd "$ROOT/../agentic_loop_template" && pwd)"
elif [[ -d "$ROOT/agentic_loop_template/memory" ]]; then
  TEMPLATE="$(cd "$ROOT/agentic_loop_template" && pwd)"
else
  echo "agentic_loop_template not found (expected ../agentic_loop_template, ./agentic_loop_template, or AGENTIX_TEMPLATE)" >&2
  exit 1
fi
