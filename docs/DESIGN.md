...
Установка и пять решений по сжатию контекста: [LOOP.md](LOOP.md). Команды: `./init`, затем `./loop`.

Пользовательская документация: [оглавление](README.md), [API](API.md), [настройки](CONFIGURATION.md), [корпус](CORPUS.md).
